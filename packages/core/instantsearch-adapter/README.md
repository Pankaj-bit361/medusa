...
There is no store search HTTP route in Medusa core yet. Point the adapter at the route you add; the default path is `/store/search`.
...
Supported: `searchBox`, `hits`, `infiniteHits`, `pagination`, `hitsPerPage`, `stats`, `configure`, `index`, `refinementList` (including searchable), `menu`, `menuSelect`, `toggleRefinement`, `currentRefinements`, `clearRefinements`, `sortBy`, `highlight` / `snippet` (when the search provider supports highlighting), `routing`.
...
- **Highlighting** is provider-dependent (available on `search-medusa`, not on Postgres today).
